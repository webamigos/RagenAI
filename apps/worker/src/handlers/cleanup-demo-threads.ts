import { type JobContext, type CleanupDemoThreadsResult } from '@ragenai/jobs';
 ...
 */
export async function cleanupDemoThreads(
  _payload: void,
  ctx: JobContext,
): Promise<CleanupDemoThreadsResult> {
  ...
  if (cleanup.skipped) {
    ctx.log.info('Demo cleanup skipped — no demo organization configured');
    return result;
  }

  ctx.log.info('Demo cleanup finished', {
