 ...
 */
export async function pruneAnalyticsRetrievals(
  _payload: void,
  ctx: JobContext,
): Promise<PruneAnalyticsRetrievalsResult> {
  const { pruneDocumentRetrievals } = ctx.steps<typeof activities>({
    retry: {
  ...
  ctx.log.info('Analytics retrieval prune finished', {
