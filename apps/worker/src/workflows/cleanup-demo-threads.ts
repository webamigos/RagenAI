...
/**
 * Nightly cleanup of the demo organization's abandoned conversations.
 *
...
export async function cleanupDemoThreads(): Promise<CleanupDemoThreadsResult> {
  const { deleteStaleDemoThreads } = proxyActivities<typeof activities>({
    retry: {
  ...
  if (result.skipped) {
    log.info('Demo thread cleanup skipped — no demo organization configured');
    return result;
  }

  log.info('Demo thread cleanup finished', {
