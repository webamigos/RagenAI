...
export const createEmbeddingsInstance = ({
  ...
  // The provider string lands on every `ai_usage` row. It stays `llm-gateway`
  // rather than becoming the real upstream: that is `ModelProvider`'s problem
  // and it owns the pricing namespace and every historical row, so it moves on
  // its own rather than as a side effect of retiring the proxy.
