...
export function DefaultRagSettingsForm({
  defaults,
}: {
  defaults: RagPipelineSettings;
}) {
  const [settings, setSettings] = useState<RagPipelineSettings>(defaults);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await saveDefaultRagSettingsAction(settings);
      setSaved(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        These defaults apply to newly created organizations.
      </p>
