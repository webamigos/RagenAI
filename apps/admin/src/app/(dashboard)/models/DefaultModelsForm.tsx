...
import { saveDefaultAllowedModelsAction } from './actions';
import { allModels, groupModelsByOrigin } from './models-config';

export function DefaultModelsForm({ defaults }: { defaults: string[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await saveDefaultAllowedModelsAction(Array.from(selected));
      setSaved(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Empty selection means no restriction — all models are allowed. These
        defaults apply to newly created organizations.
      </p>
