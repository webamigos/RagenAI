...
export function DefaultTemplatesForm({
  templates,
  defaults,
}: {
  ...
  const allSelected =
    templates.length > 0 && templates.every((t) => selected.has(t.id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await saveDefaultAllowedTemplatesAction(Array.from(selected));
      setSaved(true);
      if (timerRef.current) {
        clearTimeout(timerRef.current);
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <label className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium">
          All assistants {noneSelected && '(no restriction)'}
        </span>
      </label>
      ...
      <p className="text-xs text-muted-foreground">
        Empty selection means no restriction — all active assistants are
        available.
      </p>
