...
type LoginFormProps = {
  /**
   * Whether the installation has Google credentials configured. Decided on the
   * server by `isGoogleSignInConfigured()` and passed down, because this is a
   * client component and `process.env.GOOGLE_CLIENT_ID` is not readable here.
   *
   * A `NEXT_PUBLIC_` flag would have been the other option and is the worse
   * one: it is fixed at build time, so one image could not serve an install
   * that configures Google and one that does not.
   */
