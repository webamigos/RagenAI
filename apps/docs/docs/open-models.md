...
| Speech (TTS/STT)           | Off unless configured                | `SPEECH_PROVIDER=elevenlabs` leaves the network. `SPEECH_PROVIDER=openai` speaks OpenAI's `/v1/audio/*` API, so pointing `SPEECH_BASE_URL` at a local vLLM or Speaches keeps it on-premise. It used to follow `LITELLM_PROXY_URL`, which served no audio route and so 404'd rather than staying local. |
