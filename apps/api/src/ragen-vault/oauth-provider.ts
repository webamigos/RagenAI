...
const logger = new Logger('RagenAuthOAuthClientProvider');
...
export class RagenAuthOAuthClientProvider implements OAuthClientProvider {
  ...
  async tokens(): Promise<OAuthTokens | undefined> {
    try {
      ...
      logger.error(
        `Failed to retrieve tokens from ragen-token-vault (customerId=${this.customerId}, provider=${this.providerKey})`,
        error,
      );
      return undefined;
    }
  ...
  async clientInformation(): Promise<OAuthClientInformation | undefined> {
    ...
    try {
      ...
      logger.error(
        `Failed to retrieve client information from ragen-token-vault (customerId=${this.customerId}, provider=${this.providerKey})`,
        error,
      );
      return undefined;
    }
  ...
  // No `await` needed — this method is synchronous logic that just sets
  // `this._authorizationUrl`, but the OAuthClientProvider interface
  ...
  private async getExistingTokenData(): Promise<StoreTokenData> {
    try {
      ...
      logger.warn(
        `Failed to fetch existing token data from ragen-token-vault, using placeholder (customerId=${this.customerId}, provider=${this.providerKey})`,
        error,
      );
  ...
  async codeVerifier(): Promise<string> {
    try {
      ...
      logger.error(
        `Failed to retrieve code verifier from ragen-token-vault (customerId=${this.customerId}, provider=${this.providerKey})`,
        error,
      );
      throw new Error('No code verifier found');
    }
