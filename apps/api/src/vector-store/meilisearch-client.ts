...
const logger = new Logger('MeilisearchVectorStoreClient');
...
export class MeilisearchVectorStoreClient implements VectorStoreClient {
  ...
  async similaritySearch(
    ...
    const filterString = filter
      ? convertQdrantFilterToMeilisearch(filter)
      : undefined;
  ...
  async addDocuments(documents: VectorStoreDocument[]): Promise<void> {
    ...
    logger.log(
      `Documents added to Meilisearch: count=${documents.length} index=${this.indexName}`,
    );
  }
  ...
  async deleteDocuments(filter: object): Promise<void> {
    ...
    const filterString = convertQdrantFilterToMeilisearch(filter);
    if (!filterString) {
    ...
    logger.log(
      `Documents deleted from Meilisearch: filter=${filterString} index=${this.indexName}`,
    );
  }
