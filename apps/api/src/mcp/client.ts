...
import {
  ...
  ragenAuthClient,
} from '../ragen-vault/index.js';
import { classifyMcpTool } from '../security/mcp-tool-classifier.js';
import { shouldPauseForApproval } from '../security/tool-gating-context.js';
import { inspectToolArgs } from '../security/tool-arg-inspector.js';
 ...
 * Here it's an optional injected callback (see security/types.ts) so
 * `wrapToolsForConnector`/`createMcpToolsFromConnectors` stay plain,
 * framework-agnostic functions with no NestJS DI inside them — same
 * pattern as `trackAiUsage` in the llm/reranker slice. A real caller
 * passes `securityEventService.record.bind(securityEventService)`.
 */
 ...
 *
 * Prefers the live value from the provider registry (which reads from env)
...
export function resolveMcpServerUrl(
  ...
  logger.warn(
    `resolveMcpServerUrl: no provider definition found, falling back to stored URL (provider=${connector.provider}, storedUrl=${connector.mcpServerUrl})`,
  );
  return connector.mcpServerUrl;
}
  ...
  for (const [name, tool] of Object.entries(tools)) {
    if (typeof tool.execute === 'function') {
      ...
      if (parameters) {
        ...
        if (stillLeaks) {
          logger.warn(
            `wrapToolsForConnector: customer_id still present after strip — LLM will see it (tool=${name})`,
            {
      ...
      wrapped[name] = {
        ...
        execute: (args: Record<string, any>, options: any) => {
          ...
          logger.log('Sanitized tool args', {
            toolName: name,
            sideEffect,
          ...
          if (isWrite) {
            ...
            // needed here; see ../security/tool-arg-inspector.ts.
            ...
            if (inspection.risk !== 'low') {
              ...
              // bumps to critical on bursts of the same user triggering
              // TOOL_ARGS_HIGH_RISK.
              recordSecurityEvent?.({
            ...
            if (inspection.risk === 'high') {
              logger.warn('MCP tool call blocked: high-risk arguments', {
                toolName: name,
                score: inspection.score,
...
export async function createMcpToolsFromConnectors(
  ...
  for (const connector of connectors) {
    try {
      ...
      const wrappedTools = wrapToolsForConnector(
        tools,
        connector.customerId,
        recordSecurityEvent,
      );
      ...
      logger.log('MCP tools loaded from connector', {
