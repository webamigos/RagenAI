...
- ZARZĄD / UDZIAŁOWCY / POWIĄZANIA: all surfaced by \`get_krs_info\` in the \`powiazania\` field. Filter by \`typ\` and \`aktywne\` to answer "who is currently on the board" vs "who used to be". If \`powiazania\` is empty on a wykreślona entry, follow the previous instruction and chain back through earlier KRS forms.
