...
- TOOLS: \`lookup_company\` (search by NIP, REGON, or name fragment → returns KRS candidates), \`get_krs_info\` (full snapshot by KRS: legal form, PKD, siedziba, state flags, current zarząd/udziałowcy/powiązania, last-year financial snapshot when available), \`get_krs_history\` (HISTORICAL zarząd/udziałowcy/powiązania — past board members and shareholders that no longer appear in the current KRS entry; the right tool for wykreślone companies and "who was on the board before X?" questions), \`get_financials\` (revenue/profit/costs time-series; \`years=1\` by default reads a free inline snapshot, \`years>1\` hits historical filings).
