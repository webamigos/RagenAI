...
describe('detectLookup', () => {
  ...
  it('strips spaces and dashes from NIP', () => {
    expect(detectLookup({ NIP: '123-456-78-90' })).toEqual({ nip: '1234567890' });
  });
  ...
  it('skips empty string values', () => {
    expect(detectLookup({ nip: '', company: 'Acme' })).toEqual({ name: 'Acme' });
  });

  it('rejects a 9- or 11-digit NIP and falls back to other fields', () => {
    expect(detectLookup({ nip: '123456789', company: 'Acme' })).toEqual({ name: 'Acme' });
    expect(detectLookup({ nip: '12345678901', company: 'Acme' })).toEqual({ name: 'Acme' });
  });

  it('prefers exact-match nip column over substring matches', () => {
    expect(detectLookup({ supplier_nip: '1111111111', nip: '2222222222' })).toEqual({
      nip: '2222222222',
    });
  });
  ...
  it('returns null when only _enrichment_ keys are populated', () => {
    expect(
      detectLookup({ _enrichment_nip: '1234567890', _enrichment_nazwa_pelna: 'X' }),
    ).toBeNull();
  });

  it('ignores non-string values', () => {
    expect(detectLookup({ nip: 1234567890 as unknown as string, company: 'Acme' })).toEqual({
      name: 'Acme',
    });
  });
});
