...
export default function DocumentsListLoading() {
  return (
    <div className="flex gap-3 pb-5">
      {/* Sidebar skeleton */}
      <div className="hidden lg:block w-56 shrink-0 border-r border-gray-200 dark:border-gray-700 pr-2">
        <div className="flex flex-col gap-2 pt-1">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-7 w-full animate-pulse rounded bg-gray-200 dark:bg-gray-700"
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="mb-2 h-5 w-40 animate-pulse rounded bg-gray-200 dark:bg-gray-700" />

        {/* Top bar skeleton: search + layout toggle + spacer + Nowy folder + Dodaj dokument */}
        <div className="mb-3 flex shrink-0 items-center gap-3">
          <div className="h-9 w-52 animate-pulse rounded-md bg-gray-200 dark:bg-gray-700" />
          <div className="hidden md:block h-9 w-[72px] animate-pulse rounded-md bg-gray-200 dark:bg-gray-700" />
