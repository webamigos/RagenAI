...
**Rule**: a Turborepo `outputs` glob is a snapshot instruction, so it must name the build _product_ and nothing that merely lives in the same directory. When a framework moves its cache location, an old negation silently stops covering it — there is no warning, only a cache that grows. Two symptoms are worth wiring together: an implausibly large `.turbo/cache`, and restoring a cached build overwriting dev-server state (stale dev cache is part of the artifact, so a cache hit can hand a developer another machine's dev state). Check the artifact's contents, not just its size: `tar --use-compress-program=unzstd -tf .turbo/cache/<hash>.tar.zst`.
