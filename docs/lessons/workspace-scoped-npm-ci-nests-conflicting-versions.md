...
2. A second `--workspace=` on the _deps_ install (not prod-deps), so the package's own devDependencies — its pinned TypeScript — are available to build it.
