...
**Applies to**: any future absorption of a standalone repo into this monorepo (`ragen-token-vault`, `ragen-connectors`), and any time a workspace's dependency resolution changes shape — adding a workspace, changing a root pin, or introducing a shared package that hoists differently.
