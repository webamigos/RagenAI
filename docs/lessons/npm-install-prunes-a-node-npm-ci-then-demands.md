...
**It also deletes the directory from disk, and it keeps happening.** Twice more on 2026-09-23: adding `graphology` to `brain-core` (#1317 went red on every job) and adding `sigma` to `apps/web`. The second time the pruned node was also gone from `node_modules`, so anything loading the MCP SDK locally would fail on a missing `zod` v3 while the lockfile — once fixed — looked right. Restoring the lockfile entry is not enough on a machine that already ran the install: re-extract `zod@3.25.76` into `node_modules/@modelcontextprotocol/sdk/node_modules/zod` (check its integrity against the lockfile) or run `npm ci`. For a new dependency on a package the lockfile already has, skip `npm install` entirely and add the line to the workspace's `package.json` and its lockfile entry by hand.

**Dependabot writes the lockfile with npm 11, and npm 10 can pass a lockfile npm 11 rejects.** #1384 (a dev-dependency group bump) was red on every job at `npm ci`: Dependabot does not use the pinned npm, so its lockfile had both causes at once — the npm-11 prune (`esbuild@0.28.2`, `@smithy/signature-v4@5.7.3`) and the missing `zod@3.25.76`. Regenerating with npm 10 and re-inserting the zod node made npm 10's `npm ci --dry-run` green, and npm 11's still red on `Missing: @typescript-eslint/eslint-plugin@8.70.1`: the group bumped `apps/worker`'s `eslint-plugin` to `^8.70.1`, whose peer is `@typescript-eslint/parser ^8.70.1`, while the worker's own `parser` stayed on `^8.29.0`. npm 10 kept the old nested `apps/worker/node_modules/@typescript-eslint/*` 8.69.0 nodes — which do not satisfy `^8.70.1` — through every `--package-lock-only` run, and its own `npm ci` accepted them. Two fixes: bump the peer alongside (`parser` to `^8.70.1`), and delete the stale nested nodes from the lockfile so the pinned npm resolves them afresh. **So the dry run is two commands, not one** — `npm ci --dry-run` under the pinned npm *and* `npx npm@11 ci --dry-run` — and a Dependabot lockfile is regenerated, never fixed in place.
