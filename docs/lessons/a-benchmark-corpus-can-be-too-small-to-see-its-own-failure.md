...
_could_ see the failure into Phase A rather than a last checkbox.
...
fetches four. So the chunk holding the header was usually retrieved _alongside_
...
themselves: _"the provided fragments end at code CD-1978"_, _"the context
includes CNC turning, CNC milling and TIG welding"_ — the first rows of a
fifty-row table. Then the change under test moved the median to 13/18.

The 22-row corpus would have produced a _smaller_ delta from the same
...
2. **Read the failing answers, not the pass rate.** A model that says _"the
   fragments I was given end at row 12"_ is describing the failure under test.
...
The corresponding rule for a _result_: a delta measured on a corpus whose
