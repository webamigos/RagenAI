...
**Confirmed against the published artifact, Phase G2.** `ghcr.io/webamigos/ragen-worker:2.0.0` still contains `node_modules/@ragenai/jobs-temporal` — the symlink, and `/app/packages/jobs-temporal/dist` behind it — along with `@temporalio/client`, `common` and `proto`, from an install that names none of them. So the sentence people take from the Dockerfile's comment ("the running image ships one runtime, the adapter stays out of it") is true of the *SDK* and not of the adapter. It matters because the boot failure people will hit says so: with `WORKER_RUNTIME=temporal` the image starts, logs `runtime: "temporal"`, and exits with `Cannot find package '@temporalio/worker' imported from /app/apps/worker/dist/temporal-runtime.js` — never mentioning the adapter, which is right there. An enterprise layer that added only the adapter would change nothing.

**Corollary for a layer, not just for a cut.** The same link behaviour decides where a layer may write. `/app/node_modules/@ragenai/jobs-temporal` is a symlink into `/app/packages/`, so a `COPY` to that path follows it and lands in the base image's own package directory — applied, and resolving to the wrong `dist`. `ragen-enterprise`'s Dockerfile writes to `/app/apps/worker/node_modules/` instead, which is the nearest resolution root for both importers and shadows nothing. Check with `require.resolve` inside the built image rather than with the build log.
