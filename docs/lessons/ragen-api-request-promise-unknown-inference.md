...
**Problem**: TypeScript silently inferred every one of those calls as `Promise<unknown>`, which then propagated into every downstream consumer (`ConnectorCard.tsx`, `page.tsx`, and, in the projects case, `useSidebarLogic.ts`/`useNewThread.ts`/`Assistant.tsx`/`AssistantDropdownMenu.tsx`/both `ProjectComponent.tsx` files). `tsc --noEmit` only catches this if the check is broad enough to cover every _consumer_ file — a check scoped to just the edited action file missed 15 real downstream errors the first time.
