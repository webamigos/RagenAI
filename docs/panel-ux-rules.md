...
The twenty-six calls a designer would make, so that work in `apps/web` makes
them the same way without being told each time.

Reached from `AGENTS.md`'s Task Router. It lives here rather than inside that
file because `AGENTS.md` has a hard 32,768-byte ceiling — Codex's
`project_doc_max_bytes`, enforced by
`tests/architecture/agent-instructions-fit-the-budget.test.ts` — and had 1.7 KB
of headroom against these 4 KB. The handoff's instruction to paste it in was
written without knowing that. The four rules that must be obeyed without
reading anything are restated in `AGENTS.md` itself; the rest are here.

Source: `apps/web/design_handoff_ragen_panel/UX_RULES.md`.
