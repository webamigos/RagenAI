...
1. **One package**, and no `@temporalio/*` outside it. **True of the shipped
   artifact, not yet of the source tree.** E5 removed every declaration nothing
   imported and E6 took the SDK out of the worker image — measured, 1.19 GB to
   1.01 GB — by making each Temporal import dynamic and moving the packages to
   devDependencies, where `--omit=dev` reaches them. What remains in the source
