...
- [Tail latency roughly doubles at 20 concurrent ingests, well below the configured 50-activity concurrency cap](lessons/worker-concurrency-load-test-2026-09-05.md) — area:architecture; module:worker; topic:temporal,litellm,qdrant,load-test,concurrency,onboarding. Measured, not assumed — `maxConcurrentActivityTaskExecutions: 50` is not proof the system handles 50 concurrent ingests cleanly.
