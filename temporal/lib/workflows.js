...
exports.newEstimateAgeWorkflow =
...
const { generateRandomAge } = wf.proxyActivities({
...
async function EmbeddingWorkflow({ documentId }) {
...
async function newEstimateAgeWorkflow({ name }) {
  const result = await generateRandomAge({ name });
  return result;
}
exports.newEstimateAgeWorkflow = newEstimateAgeWorkflow;
