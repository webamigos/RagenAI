...
exports.createLiteLLMClient = createLiteLLMClient;
const platform_contracts_1 = require('@ragenai/platform-contracts');
...
function createLiteLLMClient(options = {}) {
  ...
  const logger = options.logger ?? logger_1.NOOP_LOGGER;
  const withLiteLLMRetry = (0, retry_1.createRetry)(logger);
  /** Infer the visual origin (for UI grouping) from a LiteLLM model name */
  function inferOrigin(modelId) {
  ...
  async function fetchLiteLLMModels() {
    ...
    try {
      ...
      const models = data.data
        .filter((m) => {
          const entry = platform_contracts_1.MODEL_REGISTRY[m.id];
  ...
  async function isLiteLLMAvailable() {
    try {
      const headers = {};
      if (masterKey) {
        headers['Authorization'] = `Bearer ${masterKey}`;
  ...
  async function updateLiteLLMTeam(params) {
    const body = {
      team_id: params.teamId,
    };
  ...
  async function deleteLiteLLMTeam(teamId) {
  ...
  async function addLiteLLMTeamMember(params) {
    const member = {
  ...
  async function removeLiteLLMTeamMember(params) {
    const body = {
      team_id: params.teamId,
    };
    if (params.userId) {
  ...
  async function getLiteLLMTeamInfo(teamId) {
    try {
  ...
  // --- Key Management ---
  async function generateLiteLLMKey(params) {
    const body = {
      team_id: params.teamId,
    };
    if (params.keyAlias) {
  ...
  async function deleteLiteLLMKey(keyToken) {
  ...
  // --- Spend Logs ---
  async function getLiteLLMSpendLogs(params) {
   ...
   */
  async function getLiteLLMKeyInfo(keyToken) {
    try {
      const response = await fetch(
