...
exports.ScalewayKMSService = void 0;
...
class ScalewayKMSService {
  ...
  async generateDataKey(keyId) {
    const data = await this.post(keyId, 'generate-data-key', {
  ...
  async decryptDataKey(keyId, ciphertext) {
    const data = await this.post(keyId, 'decrypt', {
      ciphertext,
    });
    if (!data.plaintext) {
  ...
  async encrypt(keyId, plaintext) {
    const data = await this.post(keyId, 'encrypt', {
...
exports.ScalewayKMSService = ScalewayKMSService;
/**
...
//# sourceMappingURL=scaleway-kms.js.map
